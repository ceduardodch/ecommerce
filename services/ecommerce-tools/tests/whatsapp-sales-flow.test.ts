...
const product = { id: "p1", variantId: "v1", sku: "OLLA", title: "Olla", description: "", category: "ollas", brand: "Eter", price: { amount: 20, currency: "USD" as const }, stock: 2, imageUrl: "", productUrl: "https://cocina.eter-niu.com/products/olla", tags: [] }
...
describe("advanceWhatsappSale", () => {
  ...
  it("envía un carrito solo después de confirmar datos", async () => {
    const createCart = vi.fn().mockResolvedValue({ cartUrl: "https://cocina.eter-niu.com/cart?session=opaque", expiresAt: "2026-01-01T00:00:00.000Z" })
  ...
  it("no duplica el carrito cuando el cliente repite la confirmación", async () => {
    const createCart = vi.fn()
    const result = await advanceWhatsappSale({ text: "sí", phone: "+5931", products: [], state: { productId: "p1", variantId: "v1", quantity: 1, city: "Quito", customerName: "Ana", cartUrl: "https://cocina.eter-niu.com/cart?session=opaque" }, quote: vi.fn(), createCart })
