...
- `services/ecommerce-tools`: HTTP and MCP tool layer, and the WhatsApp sales agent (Vicky) itself. It searches Medusa products, quotes, creates Medusa-backed conversational orders, creates PayPhone links, receives payment notifications, emits Meta catalog CSV, and drafts Meta/Marketplace copy.
...
2. Vicky looks up the customer by phone and searches kitchen products before recommending.
3. Vicky creates a quote; the quote event is recorded in Medusa CRM when a phone exists.
...
- WhatsApp is handled by the Cloud API webhook inside `ecommerce-tools`.
