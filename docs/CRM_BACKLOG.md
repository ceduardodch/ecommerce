...
| XS-3 | Followups estacionales | ✅ M | Job mensual `src/jobs/schedule-seasonal-followups.ts`: fechas fijas EC sobre segmentos con consentimiento; setea `next_followup_at` + `followup_reason: "estacional_*"` solo si no hay un followup más próximo. Auditado (jun 2026): solo agenda (no envía), respeta consentimiento y fechas más próximas, tests unit en verde. **Nota de auditoría**: Black Friday quedó COMBINADO con Navidad en una sola campaña (15-nov), no como campaña separada — funcionalmente equivalente; separar solo si el dueño quiere mensajes diferenciados por motivo. |
